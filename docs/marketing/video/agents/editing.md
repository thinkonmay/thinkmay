...
- **Intro 3.0–4.5s, outro 3.0–4.5s** — see [brand-design.md](../brand-design.md#intro--outro-scene-standards-60s-tutorials). `data-duration = outroStart + outro budget`, derived from the A-roll end — **never a fixed 55/60s floor** (fixed floors caused both an 18.8s frozen outro and a ~9s black tail in `disk-upgrade-60s_v1`). Outro fade must end at composition end; zero black frames after it
...
**Zoom coverage is mandatory, not decorative.** The full uncropped browser at 1.0× may be on screen for at most **~6 consecutive seconds** during instructional beats (establishing shots after a page change are the only exception). Every "click X" caption gets a zoom that frames X **and its containing card/panel**; cluttered pages (split-screen login, dashboard with sidebar) must be zoomed so the relevant region fills the frame. A 60s walkthrough should spend **≥50% of A-roll time at ≥1.2×**. (`disk-upgrade-60s_v1` failure: 0:04–0:40 shipped the entire browser — tiny text, split-screen distraction.)
...
## Pacing: cut dead time at the source

- **Never show character-by-character typing.** The template's `humanType` clicks the field then fills the value instantly. For legacy footage with typing, jump-cut: field clicked → hard cut to filled (masked) fields → submit. Typing must occupy **≤2s** of the final timeline.
- **No span of ≥4s** may pass without a caption, narration, camera move, or visible UI change (`DEAD_AIR` in the frame review). Bridge with a zoom-to-target or a per-span playback-rate bump.
- Install/processing waits: compress with a cut or speed ramp — never real-time spinners.

## Click ripples (mandatory on every click)

Every recorded click gets a visible pulse so the viewer never misses the interaction. The template wires this automatically: `record_shared.mjs` marks `Clicked: … | center=x,y`, `build-sync-timing.mjs` emits a `clicks` array, `apply-sync-to-html.mjs` fills `var CLICKS`, and the composition spawns a yellow expanding ring (`.click-ripple`, `#FFE066`) inside `#video-wrap` at the click's raw-video coordinates:

```javascript
tl.fromTo("#ripple-" + i,
  { scale: 0.3, opacity: 0.95 },
  { scale: 1.6, opacity: 0, duration: 0.5, ease: "power2.out", overwrite: "auto" },
  click.t - 0.05);
```

Ripples live inside `#video-wrap` so camera zooms transform them with the footage. Property tweens only — render-safe.

## Audio layers: music bed + SFX (mandatory)

Voice-only audio is a shipping blocker. Wire all layers per [brand-design.md](../brand-design.md#soundscape-mandatory-for-shipped-tutorials):

```html
<audio id="music-bed" class="clip" src="assets/audio/music-bed.mp3"
  data-start="0" data-duration="<composition duration>" data-volume="0.12" data-track-index="5"></audio>
<!-- sfx:start --> <!-- click + whoosh tags auto-generated by apply-sync-to-html.mjs --> <!-- sfx:end -->
```

- Music bed at `data-volume ≤ 0.15` — must never compete with narration
- Click SFX (`sfx-click.mp3`, vol 0.5, track 6) at every `clicks[]` timestamp
- Whoosh SFX (`sfx-whoosh.mp3`, vol 0.35, track 7) at every `popups[]` timestamp
- SFX clips are short; keep each type on its own track to avoid same-track overlap lint errors
