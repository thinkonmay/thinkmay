...
| HID input: proxy write | client DataChannel -> proxy IVSHMEM | proxy `HIDQueueImpl.Push` writes `internal.outgoing` from `NewDataShmListener` input | no polling wait | Mouse, keyboard, touch, and active gamepad input enter shared memory without a polling delay. |
| HID input: daemon read | proxy IVSHMEM -> daemon HID adapter | daemon `HIDQueueImpl.WriterPop` polls `internal.outgoing` into `HIDAdapter.Recv()` | 1 ms | This is the latency-critical wait on the active client-input path. |
| HID feedback: daemon write | daemon/guest -> daemon IVSHMEM | daemon `HIDQueueImpl.WriterPush` writes `internal.incoming` from `HIDAdapter.Send()` | no polling wait | Cursor, rumble, notification, and ping feedback are queued without a polling delay. |
| HID feedback: proxy read | daemon/proxy IVSHMEM -> client DataChannel | proxy `HIDQueueImpl.Pop` polls `internal.incoming` and sends through the multiplexer | 10 ms | Feedback should stay smoother than the cursor interpolation window without 1 kHz idle polling. |
...
* **TPM & Anti-Cheat Evasion (VM Hiding)**: Standard enterprise VMs are rapidly flagged and banned by gaming anti-cheat engines (e.g., BattlEye, Easy Anti-Cheat). To bypass architectural detection, the `vm.go` configuration acts to heavily spoof the hardware. The code strips KVM virtualization flags (`hypervisor=off`, `kvm=off`) and injects counterfeit SMBIOS hardware signatures masking the machine strictly as a physical generic Gigabyte rack server (`vendor=GIGABYTE, product=G292-Z20-00`). A software TPM 2.0 object is attached to ensure strict Windows 11 compliance. *(Note: Deep kernel-level hypervisor anti-cheats like Vanguard cannot be bypassed and will block game booting)*.
* **GPU VFIO Passthrough**: Found in `pcie.go`, graphic cards bypass OS virtualization completely via PCI-E passthrough (`vfio-pci`). The system iterates through the Host's `/sys/bus/pci/drivers/vfio-pci` directory to map physical graphical nodes (and their internal consumers like GPU audio interfaces) dynamically into the Windows OS. Guest-side capture targets the passthrough GPU via Sunshine (not QEMU VNC). See [Windows display & capture](./windows_display_capture.md) for the EDID dongle primary path and [Thinkmay VDD fork spec](./thinkmay_vdd_fork_spec.md) for the software fallback replacing Parsec VDD.
...
### 17. Volume Templating Pipeline
To rapidly provision new games or templates without performing heavy IO clone operations across 100GB+ drives, the Thinkmay architecture supports an instant two-phase `Set Template` API available strictly to users residing in the `_superusers` collection.
* **Phase 1: Sub-Millisecond Rename**: Using atomic `mv` OS-level operations, the daemon instantly transforms a persistent user volume into a reusable template (`<volumeid>` -> `<code_name>.template`). To prevent data loss, the daemon first asserts if the `<code_name>.template` already exists. If it does, it dynamically backups the existing template by appending `.old` to its path, overriding any previous backups cleanly.
* **Phase 2: Automated Clean Allocation**: Once the target volume is transformed into a template, the user's VM is left without a backing drive. The API immediately triggers the standard allocation process, fetching a clean `win11.template` and dropping it into the user's now-empty `<volumeid>` slot, flawlessly resetting their OS back to factory defaults while capturing their customized template.
...
### 19. Personal Cloud Storage & Security (Storj)
The ecosystem employs a robust Zero-Trust model for extending native file-sharing and auto-attaching isolated User Storage directly into the running virtual machines. Instead of using centralized storage like standard AWS S3, Thinkmay maps user drives onto the heavily decentralized **Storj** network via customized Go wrapper integration (`app/storj/storj.go`).

* **Auto-Attaching Buckets (`/new` API)**: Whenever a client signals the daemon to boot their CloudPC (via the Pocketbase `/new` HTTP flow), the backend locates their `c.Auth.Id`. It immediately scans the local `buckets` collection mapping their User ID directly to their reserved Storj bucket. The server executes `GrantCredential(bucketName)` natively allocating an **Ephemeral Connection Token**. The node injects this isolated temporary token directly across internal networks straight to the QEMU host proxy, auto-attaching the drive in the VM without manual sign-ins.
* **Direct Edge Downloading (`/file/v1` API)**: The web dashboard allows users to manage remote items directly from the Cloud storage pool natively out of the browser. To eliminate proxy bottlenecks routing massive zip files across the backend daemon's bandwidth, whenever a user executes `pb.download()`, the daemon intercepts the request. It calculates a native `uplink.SharePrefix` bound exclusively to the singular requested file and requests a secure **Edge Authorization Link** (`DownloadableURL()`). The Go backend immediately returns an HTTP 307 Redirect pointing the browser seamlessly out to `link.storjshare.io` allowing the user to seamlessly download enormous volumes directly off the decentralized CDN edge. 
* **Data Scoping & Protection**: A user can unconditionally *never* accidentally crawl into another user's drive system. The internal `GrantCredential({})` mechanism natively locks their access via strict `SharePrefix: bucketName` parameters and logically expires backend connection tokens via hard timeouts `NotAfter: Time.Now().Add(Expiration)`. Furthermore, the Storage Gateway endpoints are heavily shielded using native Pocketbase user authentication locks before hitting the `uplink` driver.
...
The Thinkmay PostgreSQL schema (`docs/shared/data/db/global.sql`) institutes a heavily dynamic hybrid billing layer tracking explicit Subscriptions against Base Allowances and Service Overages.
