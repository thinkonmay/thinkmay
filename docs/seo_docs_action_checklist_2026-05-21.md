...
- [x] Add a live-GSC confirmation note that:
  - Cloud PC remains the strongest SEO position
  - rental-intent terms have traction
  - Indonesia is already a meaningful organic market
...
- [ ] Review the `trumbox` query cluster and determine whether it comes from:
  - legacy content
  - poor query/page matching
  - unrelated ranking inheritance
- [ ] Run a dedicated page/query mapping for:
  - `/discovery/`
  - `/en/discovery/`
  - `/pricing/`
  - `/login/`
- [ ] Prioritize pages around:
  - `cloud pc`
  - `thuê cloud pc`
  - `cloud gaming`
  - Indonesia entry pages

### Measurement
- [ ] Add a weekly workflow that separates:
  - real product-intent opportunities
  - irrelevant-query cleanup
- [ ] Join GSC query/page data with page-level Rybbit landing-page data once that export is available.

## Current strategic interpretation

If team bandwidth is limited, the best next sequence is:

1. Protect and expand `cloud pc`
2. Build around `thuê cloud pc` and related commercial rental intent
3. Fix `/discovery/` and especially `/en/discovery/`
4. Protect `/id/` and improve Indonesia-localized paths
5. Investigate and isolate irrelevant query noise before broad CTR optimization work
